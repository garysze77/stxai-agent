...
func ShowNodeHeader(node string, lang string) {
	...
	case strings.Contains(node, "bullish"):
		bullColor.Printf("\n  ── %s ──\n\n", i18n.T("cli.display.bull_case", lang))
	case strings.Contains(node, "bearish"):
		bearColor.Printf("\n  ── %s ──\n\n", i18n.T("cli.display.bear_case", lang))
...
func ShowNodeContent(node string, data json.RawMessage, lang string) {
	...
	case strings.Contains(node, "signal"):
		...
		if bias, ok := payload["directional_bias"]; ok {
			...
			if s != "" {
				signalColor.Printf("\n  %s: %s", i18n.T("cli.display.direction", lang), s)
			}
		}
		if score, ok := payload["confidence_score"]; ok {
			...
			if n > 0 {
				signalColor.Printf(" | %s: %.0f/100", i18n.T("cli.display.confidence", lang), n)
			}
		}
		if strength, ok := payload["signal_strength"]; ok {
			...
			if s != "" {
				signalColor.Printf(" | %s: %s", i18n.T("cli.display.strength", lang), s)
			}
		}
		fmt.Println()
	}
...
func ShowError(data json.RawMessage, lang string) {
	...
	errorColor.Printf("\n  %s\n", i18n.T("cli.display.error_in", lang, node, errMsg))
}

func ShowHeartbeat(data json.RawMessage) {
	// Heartbeats are handled by the spinner
}
